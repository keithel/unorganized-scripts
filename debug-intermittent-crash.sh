...
# The example 'crash' program I refer to here is the following C++ program:
##include <iostream>
##include <string>
##include <cstdlib>
##include <ctime>
...
trap 'exec 2>&4 1>&3; echo Ctrl-C pressed, exiting loop. ; rm ${lldbpy_path}.py >/dev/null 2>&1; exit' SIGINT SIGTERM
...
echo "Starting looped debugging of app."
...
for i in {1..1000}; do
    if [ ${uname_o} == "Msys" ]; then
        ...
        rust-gdb -iex='set pagination off' -ex='set confirm on' -ex run -ex=quit --args ./crash
    fi
    echo ${i}
done
exit 0
